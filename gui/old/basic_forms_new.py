...
class BasicForm(UIBuilder):
	def __init__(self, parent, db_conn, table_name, title, *args, combo_dict=None, disabled=None, layout_file="layouts/basic_form.json", **kwargs):
		...
		tab_names = [self.parent.notebook.tab(i, option="text") for i in self.parent.notebook.tabs()]
		if title in tab_names:
			self.parent.notebook.select(tab_names.index(title))
			return
		root = ttk.Frame(self.parent.notebook)
	...
	def on_cancel(self):
		print(f"Window size = {self.root.winfo_height()}x{self.root.winfo_width()}")
		print(f"Define tab closing logic!")

def PartyForm(root, db_conn, *args, **kwargs):
	pf = BasicForm(root, db_conn, "party_db", "Party Manager", *args, combo_dict={"ACC_TYPE":{"current":"sale", "values":["sale","purc"]}}, disabled = ["ALIAS"], **kwargs)
...
def ProductForm(root, db_conn, *args, **kwargs):
	pf = BasicForm(root, db_conn, "product_db", "Product Manager", *args, disabled = ["PID"], **kwargs)
	pf.build()
	return pf
...
def FyForm(root, db_conn, *args, **kwargs):
	pf = BasicForm(root, db_conn, "fy_list", "Fy Manager", disabled = ["FY_ID"], *args, **kwargs)
...
class TransactionForm(BasicForm):
	def __init__(self, root, db_conn, billno, sr_no, trsc_type, fy_id, *args, **kwargs):
		trsc_name = f"{trsc_type}_fulldata_{fy_id}"
		super().__init__(root, db_conn, trsc_name, *args, layout_file="layouts/transaction_form.json", disabled = ["TID", "BILL", "SR_NO", "PID", "EXPIRY", "MRP"], combo_dict = {"BATCH":{"current":"", "values":[]}}, **kwargs)
...
if __name__=="__main__":
	import sys
	root = tk.Tk()
	func_map = {"fy" : ["FyForm", {"root":root,"db_conn":db_conn}],
				"party" : ["PartyForm", {"root":root,"db_conn":db_conn}],
				"product" : ["ProductForm", {"root":root,"db_conn":db_conn}],
				"transaction":["TransactionForm", {"root":root, "billno":1, "sr_no":1, "trsc_type":"sale", "fy_id":"fy_25","db_conn":db_conn}]}
