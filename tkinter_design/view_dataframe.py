...
class create_treeview:
	def __init__(self,window, main_data, show_cols='all', col_widths=None, ro=0, col=0, rospan=1, colspan=4, out='', scrollbar=None, font=main_font):
		self.ro = ro
		self.col = col
		...
		if self.col_widths is None:
			self.col_widths = dict(zip(self.main_data, [20*len(name0) for name0 in list(self.main_data.columns)]))


		if self.show_cols=='all':
			self.data = main_data
		else:
			self.data = main_data[self.show_cols]
	...
	def create_table(self,datlist=None):
		...
		for name0 in list(datlist.columns):
			...
			self.treeview.column(name0, width=self.col_widths[name0])  # Adjust the width as needed
