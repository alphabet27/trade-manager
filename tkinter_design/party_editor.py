...
class party_class:
	def __init__(self,frame,party_path,**kwargs):
		self.frame = frame
		self.party_path = party_path
		self.party_class = manage_party(party_path)
		...
		self.ss1 = multi_selector(self.frame, self.party_class.df_party, ro = self.ro, col=self.col, addnone=True)
		...
		self.ee1 = entries(self.frame, hlist = list(self.party_class.df_party.columns), ro=self.ro, col=self.col)
		self.ee1.labelize()
		...
		self.canc_button = tk.Button(self.frame, text='Cancel', font=self.font, command=lambda:self.window.destroy())
	...
	def to_save(self):
		...
		if self.mode == 'add':
			...
			#ind = self.party_class.get_index(new_var)
			#if ind:
				#del new_var['ID00']
				#self.party_class.df_party.loc[ind,new_var.keys()] = new_var.values()
			#try:
			...
			self.party_class.drop(self.party_class.get_index(self.ss1.out))
			self.party_class.save()
			self.ee1.disable_entries()
		else:
...
if __name__=='__main__':
	...
	upper_menu = menubar(window,filename='party_latest.py')
	
	party_path = '../scripts/sample_database/party_list.csv'
	
	party_class = party_class(window,party_path,font=Font_tuple)
