...
class entries:
	def __init__(self,window,hlist=[],elist=[],ro=0,col=0,numerics=None,floats=None,font=Font_tuple):
		self.numerics = list([])
		self.floats = list([])
		self.window = window
		self.hlist = hlist
	...
	def get_entries(self):
		...
		for i in range(len(self.hlist)):
			if i in self.numerics:
				if i in self.floats:
					ent_list = ent_list + [float(getattr(self,'ent'+str(i)).get())]
				else:
					ent_list = ent_list + [float(getattr(self,'ent'+str(i)).get())]
			else:
	...
	def labelize(self):
		for i in range(len(self.hlist)):
			...
			if i>0:		# Focus shifting event
				getattr(self,'ent'+str(i-1)).bind('<Return>',lambda event:getattr(self,'ent'+str(i)).focus_set())
		self.relabel()
		
	def make_numeric(self,index,allowfloat=False):
		def callback(index):
			string = getattr(self,'ent'+str(index)).get()
			ndots = string.split('.')
			case0 = (len(ndots)>2)
			case1 = (False in [((ndots[_].isnumeric()) or (ndots[_]=='')) for _ in range(len(ndots))])
			if (((not allowfloat) and (not string.isnumeric())) or (allowfloat and (case0 or case1))):
				getattr(self,'ent'+str(index)).delete(0,tk.END)
				print('Locals string, ndots, case0, case1 :',string,ndots,case0,case1)
