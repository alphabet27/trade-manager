...
class transactions:
	def __init__(self,database,mode='sale'):
	...
	def billdata(self,billno):
		if not billno==0:
			...
			self.df_bill = (df_bill.drop(df_bill.columns[0],axis=1).merge(self.product_map[['ID00','PRODUCT0']]))[['REF_NO','ID00','PRODUCT0','QTY','RATE','GST','DISC','BATCH00','EXP00']] #,'TAXABLE']]
			...
			df_bill = pd.DataFrame(columns=list(self.df_transaction.columns)+['PRODUCT0']+['TAXABLE'])
			self.df_bill = df_bill.drop(df_bill.columns[0],axis=1)[['REF_NO','ID00','PRODUCT0','QTY','RATE','GST','DISC','BATCH00','EXP00','TAXABLE']]
			self.curr_billdata = None
